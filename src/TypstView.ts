...
export class TypstView extends TextFileView {
    ...
    public async exportToPdf(): Promise<void> {
        ...
        try {
            ...
            const pdfData = await this.plugin.compileToPdf(content, filePath, "export");
            if (!pdfData) {
    ...
    private showSourceMode(): void {
        ...
        this.typstEditor = new TypstEditor(contentEl, this.app, this.plugin, (content: string) => {
    ...
    private async showReadingMode(pdfData: Uint8Array): Promise<void> {
        ...
        try {
            await this.pdfRenderer.renderPdf(pdfData, readingDiv, this.plugin.settings.enableTextLayer);
