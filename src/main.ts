...
export default class TypstForObsidian extends Plugin {
    ...
    async loadFonts() {
                        ...
                        for (const file of files) {
                            ...
                            if (
                                missingFonts.some((cf) => {
                                    const configured = cf.toLowerCase();
                                    return (
                                        fileLower.includes(configured) || configured.includes(fileBase)
                                    );
                                })
                            ) {
                                try {
        ...
        if (loadedFonts.length > 0) {
            this.compilerWorker.postMessage({ type: "fonts", data: loadedFonts }, loadedFonts);
        }
    ...
    async handleWorkerRequest({ buffer: wbuffer, path }: WorkerRequest) {
        try {
            ...
            if (actualPath.startsWith("@")) {
                const text = await this.packageManager.preparePackage(actualPath.slice(1));
                if (text) {
                    ...
                    const dataView = new Uint8Array(wbuffer.buffer, 8, encoded.byteLength); // Added length parameter
                ...
                if (text) {
                    ...
                    wbuffer[1] = encoded.byteLength;
                    const dataView = new Uint8Array(wbuffer.buffer, 8, encoded.byteLength);
                    dataView.set(encoded);
