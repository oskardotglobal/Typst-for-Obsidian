...
export class PackageManager {
    ...
    private async fetchPackage(folder: string, name: string, version: string): Promise<void> {
        ...
        await (untarrer as any).progress(async (file: any) => {
            ...
            if (file.type === "0") {
                await this.plugin.app.vault.adapter.writeBinary(folder + file.name, file.buffer);
            }
        });
    }
